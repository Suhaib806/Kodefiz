        ...
        <div className="sticky top-0">
          <div className="px-8 md:px-12 lg:px-20 py-20">
            <div className="flex flex-col items-center text-center">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-navy-200 text-xs text-navy-500">
                Why us?
              </div>
              <h2 className="mt-6 text-[44px] md:text-[56px] leading-[1.05] font-display font-normal text-navy-950">
                Why partner with us
              </h2>
              <p className="mt-3 text-sm text-navy-500">Your goals, our creative solutions</p>
            </div>

            {/* Visual */}
            <div className="relative mt-14 mx-auto w-[520px] h-[520px] md:w-[560px] md:h-[560px]">
              {/* circle */}
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-[420px] w-[420px] md:h-[460px] md:w-[460px] rounded-full bg-gradient-to-b from-navy-200 to-navy-50" />

              {/* dotted ring */}
              <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                <div className="h-[520px] w-[520px] md:h-[560px] md:w-[560px] rounded-full border border-dashed border-navy-200" />
              </div>
              ...
              >
                <div className="rounded-[28px] overflow-hidden border border-navy-200 shadow-card-hover bg-white">
                    ...
                    <div className="flex items-start gap-3">
                      <span
                        className={`mt-1 h-2.5 w-2.5 rounded-full ${
                          isActive ? "bg-brand" : "bg-navy-300"
                        }`}
                      />
                      <div className={`font-medium ${isActive ? "text-navy-950" : "text-navy-300"}`}>
                        {s.title}
                      </div>
                    </div>
                    <div
                      className={`mt-2 text-sm leading-[18px] ${
                        isActive ? "text-navy-500" : "text-navy-200"
                      }`}
                    >
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-xl border px-5 py-4 transition-colors ${
                        isActive ? "border-brand bg-navy-50" : "border-navy-200 bg-white"
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <span className={`h-2.5 w-2.5 rounded-full ${isActive ? "bg-brand" : "bg-navy-300"}`} />
                        <div className="font-medium text-navy-950">{s.title}</div>
                      </div>
                      {isActive && <div className="mt-2 text-sm text-navy-500">{s.description}</div>}
                    </div>
                  );
                })}
              </div>
