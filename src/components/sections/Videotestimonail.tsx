...
const testimonials = [
  {
    ...
    quote: "Kodefiz delivered beyond our expectations. Our new site converts 3x better.",
    rating: 5,
  },
  {
    id: 2,
...
const StarRating = ({ count }) => (
  <div className="flex items-center gap-1">
    {Array.from({ length: count }).map((_, i) => (
      <svg key={i} className="w-3.5 h-3.5 fill-brand" viewBox="0 0 20 20">
  ...
  return (
    <div
      ...
      className={`relative rounded-2xl overflow-hidden cursor-pointer transition-all duration-500 select-none ${
        isActive
          ? "ring-2 ring-brand/60 shadow-brand"
...
const VideoTestimonials = () => {
  ...
  return (
    <section className="bg-navy-950 text-white w-full px-4 sm:px-6 md:px-12 lg:px-20 py-20 sm:py-28 overflow-hidden">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-14">
        <div>
          <p className="text-brand text-[13px] uppercase tracking-[0.12em] font-medium mb-3">
            Client Stories
          </p>
          <h2 className="font-display text-[36px] sm:text-[52px] md:text-[64px] leading-[0.95] tracking-[-0.03em]">
            Heard it from
            <br />
            <span className="text-brand">them first.</span>
          </h2>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={prev}
            className="w-11 h-11 rounded-full border border-white/20 flex items-center justify-center hover:border-brand/60 hover:text-brand transition-colors duration-300"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
        ...
        <div className="order-2 lg:order-1 flex flex-col justify-center gap-8">
          <Quote className="w-10 h-10 text-brand/40" />
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  i === active ? "w-8 bg-brand" : "w-1.5 bg-white/20 hover:bg-white/40"
                }`}
              />
            ))}
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 pt-4 border-t border-white/10">
            {[
              ...
              <div key={stat.label}>
                <p className="text-brand text-[22px] sm:text-[28px] font-medium leading-none">
                  {stat.value}
                </p>
