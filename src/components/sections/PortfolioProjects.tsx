...
const CardShell = ({
  ...
  <div
    className={cn(
      "group relative overflow-hidden rounded-2xl sm:rounded-3xl bg-navy-950 border border-navy-200 shadow-sm",
      className
    )}
  >
    {children}
...
const CaseStudyCta = ({ href }: { href?: string }) => (
  <a
    ...
    className="inline-flex items-center gap-2 rounded-full bg-white pl-5 pr-1.5 py-1.5 text-sm font-semibold text-navy-950 shadow-lg hover:bg-navy-50 transition-colors"
  >
    View live site
...
const ImageCard = ({
  ...
  <CardShell className={className}>
    ...
    />
    <div className="absolute inset-0 z-[2] bg-gradient-to-t from-navy-950/60 via-transparent to-transparent opacity-80 pointer-events-none" />
      ...
      <div className="relative z-[1] w-full px-4 sm:px-6 md:px-12 lg:px-20">
        <div className="w-full mx-auto">
          <header className="text-center pt-2 sm:pt-4 pb-12 sm:pb-14 lg:pb-16">
            <div className="inline-flex items-center gap-2 rounded-full bg-navy-950/[0.06] px-4 py-2 text-xs font-medium uppercase tracking-[0.12em] text-navy-950 border border-navy-200">
              <span className="h-1.5 w-1.5 rounded-full bg-brand" />
              Our Projects
            </div>
            <h2 className="mt-6 font-display text-[34px] sm:text-[44px] lg:text-[52px] font-bold leading-[1.08] text-navy-950 max-w-[720px] mx-auto">
              Innovation and creativity in action
            </h2>
            <p className="mt-4 text-[15px] sm:text-[17px] text-navy-500 max-w-[560px] mx-auto leading-relaxed">
