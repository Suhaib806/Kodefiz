...
const TermsAndConditions = () => {
  return (
    <main className="min-h-screen text-navy-950">
      {/* Hero */}
      <section className="w-full px-4 sm:px-6 md:px-12 lg:px-20 pt-24 pb-16 border-b border-navy-200 bg-navy-950">
        <p className="text-brand text-[13px] uppercase tracking-[0.12em] font-medium mb-4">
          Legal
        </p>
        ...
        <p className="mt-6 text-white/60 text-[16px] leading-relaxed max-w-xl">
          ...
          our website or engaging with Kodefiz, you agree to the following.
        </p>
        ...
        <div className="max-w-4xl space-y-0">
          {sections.map((section, index) => {
            ...
            return (
              <div
                key={index}
                className="group flex gap-6 sm:gap-10 py-10 border-b border-navy-200 last:border-b-0"
              >
                {/* Number + Icon */}
                <div className="flex flex-col items-center gap-3 flex-shrink-0">
                  <span className="text-navy-400 text-[12px] font-mono tabular-nums">
                    {String(index + 1).padStart(2, "0")}
                  </span>
                  <div className="w-10 h-10 rounded-full border border-navy-200 flex items-center justify-center group-hover:border-brand/50 transition-colors duration-300">
                    <Icon className="w-4 h-4 text-navy-400 group-hover:text-brand transition-colors duration-300" />
                  </div>
                </div>

                {/* Content */}
                <div className="flex-1 pt-1">
                  <h2 className="text-[18px] sm:text-[20px] font-medium text-navy-950 leading-snug mb-4">
                    {section.title}
                  </h2>
                  <p className="text-[15px] sm:text-[16px] text-navy-600 leading-relaxed">
                    {section.content}
                  </p>
                </div>
              </div>
      ...
      {/* CTA */}
      <section className="w-full px-4 sm:px-6 md:px-12 lg:px-20 py-16 border-t border-navy-200">
        <div className="max-w-4xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
          <div>
            <h3 className="text-[22px] sm:text-[28px] font-medium leading-snug text-navy-950">
              Still have questions?
            </h3>
            <p className="text-navy-500 text-[15px] mt-2">
              We're happy to clarify anything you're unsure about.
            </p>
          </div>
          <a
                  href="mailtoKodefiz@gmail.com:"
